...
class BankAccountControllerSpec extends BaseSpec {
  ...
  "The handleEvent action" when {
    ...
    "valid JSON is received" should {
      ...
      "return 200" in {
        val result: Result = controller.handleEvent(request.withJsonBody(validDesRequestJson("Repayment Bank Account Change")))
